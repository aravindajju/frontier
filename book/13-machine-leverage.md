...
The demo that opened week two at Granite Mutual was, by every standard demos are judged on, a success. Six property claims, summarized by the platform's triage agent: a hail-damage file compressed from forty pages to a paragraph an adjuster could act on, coverage lines identified, a recommended priority. Hal Brennan, the VP of Claims sponsoring the engagement, watched his staff pass a printout around like a good report card. Adit's private reaction was the one this chapter exists to defend: six claims is six draws from a distribution, and nobody in the room knew what the distribution looked like.

Building the answer took two days — the platform's eval harness, four hundred adjudicated historical claims with known outcomes pulled with the intake team's help, his agents doing the labeling scaffold while he defined what correct meant in the Charter's terms. The suite mostly confirmed the demo: summaries accurate, coverage identification strong. Then it hit the subrogation files — thirty-one of the four hundred, claims where Granite pays its insured and pursues recovery from a third party — and in ten of the thirty-one, the summary had the direction of liability inverted. Carrier owes, where the carrier was owed. The sentences read fluently either way; an adjuster skimming at triage speed would deprioritize exactly the files where money was waiting to be recovered. No curated demo would have surfaced it, because nobody curates a demo around subrogation, and the model failed confidently in both directions.

The fix was context engineering, not model surgery — feed the subrogation flag and the party-role table to the summarizer explicitly, add a directionality check to every subrogation summary — and it took a day. What mattered was when it happened: week three of eight, caught by a table of numbers, instead of month three of production, caught by a recovery clerk wondering why the pipeline had gone quiet. From that week on, the eval score was the line Brennan saw at every Showcase. He never asked for another demo.
